<form method="post" action="<?=base_url()?>users/roles/create" class="modal-wrapper column-8" validate-form="true" validation-error="<?=$this->lang->line("please_check_marked_fields")?><br/><?=$this->lang->line("make_sure_at_least_one_primary_action_should_be_allowed")?>">
	<div class="modal-header">
		<?=$this->lang->line("create_role")?>
	</div>	
	<div class="modal-content">
		<div class="inline-form-row">
			<div class="column-4">
				<label><?=$this->lang->line("copy_role")?></label>
			</div>
			<div class="column-4">
				<select class="full-width" onchange="roles.copyRole(this);">
					<option value=""><?=$this->lang->line("select_role")?></option>
					<?php
					for($r=0;$r<count($all_roles);$r++){
					?>
					<option value="<?=$all_roles[$r]->id?>" <?=$copy?($item->id==$all_roles[$r]->id?"selected=\"selected\"":""):""?>><?=$all_roles[$r]->name?></option>
					<?php
					}
					?>
				</select>
		...
		<div class="inline-form-row">
			...
			<div class="column-4">
				<input type="checkbox" id="full_access" name="full_access" value="1" />
			</div>
			<div class="clearfix"></div>
		</div>		
		<div class="inline-form-row">
			...
			<div class="column-8">
				...
				<div class="permission-module-block">
					<div class="permission-box">
						...
						<div class="column-6">
							<input type="checkbox" id="show_in_navigation_<?=$i?>" name="permissions[<?=$modules[$i]->name?>][show_in_navigation]" value="1" />
					...
					<div class="permission-sections-block" id="hideable-sections-block-<?=$i?>" style="display:none;">
						...
						<div class="permission-actions-block" id="hideable-actions-block-<?=$i?>-<?=$s?>" style="display:none;">
							...
							<div class="permission-box<?=$a==count($sections[$s]->actions)-1?" last-permission-box":""?>">
								...
								<div class="column-6">
									<input type="checkbox" id="allow_action_<?=$i?>_<?=$s?>_<?=$a?>" name="permissions[<?=$modules[$i]->name?>][<?=$sections[$s]->name?>][]" value="<?=$sections[$s]->actions[$a]->name?>" onchange="roles.checkAllActions();" <?=$a==0?"required-field=\"true\" validation=\"[roles-check-primary-action:primary-action]\"":""?>/>
					...
					<div class="permission-sections-block" id="hideable-sections-block-<?=$i?>" style="display:none;">
						...
						<div class="permission-actions-block" id="hideable-actions-block-<?=$i?>-<?=$s?>" style="display:none;">
							...
							<div class="permission-box<?=$a==count($sections[$s]->actions)-1?" last-permission-box":""?>">
								...
								<div class="column-6">
									<input type="checkbox" id="allow_action_<?=$i?>_<?=$s?>_<?=$a?>" name="permissions[<?=$modules[$i]->name?>][<?=$sections[$s]->name?>][]" value="<?=$sections[$s]->actions[$a]->name?>" onchange="roles.checkAllActions();" <?=($this->acl->checkPermissions($modules[$i]->name,$sections[$s]->name,$sections[$s]->actions[$a]->name,$item->id))?"checked=\"checked\"":""?> />
									<label for="allow_action_<?=$i?>_<?=$s?>_<?=$a?>"><?=$this->lang->line("allow_action")?></label>		
								</div>
								<div class="clearfix"></div>							
							</div>															
		...
		?>		
		<?php
		$this->event->register("RoleCreateFormRow");
		?>
